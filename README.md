...
本 Fork 会在构建时为已审查的五位服务短号增加 `021` 前缀的来显匹配别名。例如，`10016` 同时匹配 `02110016`，`95533` 同时匹配 `02195533`。原始号码保持不变；新增号码仅用于处理上海运营商的来显形式。号码保存为纯数字，iPhone 来显中的括号与空格不影响匹配。

上游数据会每天自动同步；有更新时会触发新的 Release。注意：官方的 `vcards.metowolf.com` CardDAV 服务仍只提供原版数据。本 Fork 的 Release 可下载 `汇总.vcf` 手动导入；如需自动订阅，则需自行部署 CardDAV 服务。
